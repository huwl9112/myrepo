						...
						<dl>
							<dt>
								...
								<c:forEach items="${c.categorySeconds}" var="cs">
									<dd>
										<a href="${pageContext.request.contextPath}/productfindByCsid.action?csid=${cs.csid }&page=1">${cs.csname }</a>
									</dd>
								</c:forEach>
						</dl>
					</c:forEach>
	...
	<div class="pagination">
		第  ${pageBean.page}/${pageBean.totalPage}页
			<c:if test="${ pageBean.page != 1}">
				<a href="${ pageContext.request.contextPath }/productfindByCsid?csid=${csid }&page=1" class="firstPage">&nbsp;</a>		
				<a href="${ pageContext.request.contextPath }/productfindByCsid?csid=${csid }&page=${pageBean.page-1}" class="previousPage">&nbsp;</a>	
			</c:if>	
			<c:forEach var="i" begin="1" end="${pageBean.totalPage}" step="1">
				...
				<c:if test="${pageBean.page!=i }">
					<a href="${ pageContext.request.contextPath }/productfindByCsid?csid=${csid }&page=${i}">${i}</a>
				</c:if>
			</c:forEach>
				
			<c:if test="${pageBean.page != pageBean.totalPage }">
				<a class="nextPage" href="${ pageContext.request.contextPath }/productfindByCsid?csid=${csid}&page=${pageBean.page+1}">&nbsp;</a>
				<a class="lastPage" href="${ pageContext.request.contextPath }/productfindByCsid?csid=${csid}&page=${pageBean.totalPage}">&nbsp;</a>
			</c:if>	
	</div>
			</form>
		</div>
